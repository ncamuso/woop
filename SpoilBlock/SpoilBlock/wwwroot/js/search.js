...
function buildSearchResults(data) {
    ...
    let searchTable = $('<table class="table table-dark" id="resultsTable"><tbody>');
    $.each(results, function (i) {
        let tr = $(`<tr><td><div class="img-max"><img src=${results[i].image} class="img-fluid" style="max-width:100px;"></div></td><td class="mediatitle">${results[i].title}</td><td class="mediadescription">${results[i].description}</td><td><input type="submit" value="Add" id="${results[i].id}" class="btn btn-primary" data-title="${results[i].title}"/></td></tr>`).appendTo(searchTable);
    });
    $('#resultsTableDiv')
        .empty()
        .append(searchTable);
